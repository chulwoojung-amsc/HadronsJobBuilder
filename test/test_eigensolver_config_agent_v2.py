...
from femtomeas.meas_config_agent_v2.eigenvectors import setupEigenSolvers
...
if __name__ == "__main__":
    ...
    state.actions = f"""
actions = [ {action_inst.model_dump_json()} ]
"""
    use_inst = InstanceInfo(instance_tag="action_inst", user_info="")
    state.observable_actions = { "pion2pt" :  f"pion2pt_actions = [{use_inst.model_dump_json()}]" }

    while(1):
        invalidate_later_stages = setupEigenSolvers(llm, "pion2pt", "", state)
        print("EIGENSOLVERS", state.eigensolvers)
        print("OBS EIGENSOLVERS", state.observable_eigensolvers["pion2pt"])
