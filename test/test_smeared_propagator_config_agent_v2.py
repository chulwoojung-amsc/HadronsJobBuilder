...
from femtomeas.meas_config_agent_v2.smeared_prop_config import identifySmearedPropagators
...
"""
    ...
    use_prop_insts = [ InstanceInfo(instance_tag="prop_1", user_info="") ]
    state.observable_propagators = { "pion2pt" :  f"pion2pt_propagators = [{ ", ".join([ str(a.model_dump()) for a in use_prop_insts ] )}]" }

    invalidate_later_stages = identifySmearedPropagators(llm, "pion2pt", "", state)
    print("SMR PROPAGATORS", state.smeared_propagators)
    print("OBS SMR PROPAGATORS", state.observable_smeared_propagators["pion2pt"])
