...
from .solver_config_models import SolverConfig
from .state import State
    
def identifySolvers(model, observable_tag:str, observable_info:str, state: State):    
    role = """identifying the solvers required for computing the lattice QCD propagators for the observable.
...
Previous agent interactions have identified a set of observables and their required number of propagators. Solvers are required to compute those propagators. A solver instance has a set of parameters such as stopping conditions and the maximum number of iterations. The instance also has an 'action' field, that must be set to the name of one of the action instances identified previously. Each action instance must have one or more solver instances associated with it."""
  ...
  - Solver instances should be reused wherever possible. Do not insist that each propagator have a unique solver.
  ...
  Examples:
    ...
    user_info_rules = """- You must summarize any information relevant to what observables/propagators this solver will be used for provided by the user.
...
- For example, if the user specifies that this solver will be used for light quark propagators, enter "use for all light quark propagators" in user_info."""
    ...
    info = ""
    if observable_info != "":
        info = f"The following information is known about the solvers for this observable:\n{observable_info}"
...
"""
    ...
    def checkAll(solvers):
        ...
        #Check it only used actions previously described as being associated with this observable
